...
class Qlearning:
    ...
    def epslion_greedy(self, state, possible_moves,display): #esplion greedy algorithm
        #return  action
        self.last_board = tuple(state)
        if display:
            utl = np.array([float("-inf")]*64)
            for i in possible_moves:
                num = self.getQ(tuple(state), i)
                if num != 0:
                    utl[i-1]= round(num, -int(math.floor(math.log10(abs(num))) - (3 - 1)))
                else:
                    utl[i-1]= 0
            print(np.reshape(utl,(4,4,4)))
            print('-'*30)
